...
students.image as `image` FROM students WHERE students.id=chat_users.student_id) ELSE (SELECT staff.image as `image` FROM staff WHERE staff.id=chat_users.staff_id) END) as image, history.`event_time` AS history_time FROM `chat_users` left JOIN students on students.id=chat_users.student_id left JOIN staff on staff.id=chat_users.staff_id LEFT JOIN history ON chat_users.`user_type` = history.`user_type` AND (CASE WHEN (chat_users.`user_type`='student') THEN chat_users.`student_id` = history.`user_id` ELSE staff_id = history.`user_id` END) WHERE chat_users.id=" . $chat_user_id;
        $query = $this->db->query($sql);
        $chat_user = $query->row();
