...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' order by send_notification.publish_date desc");
    ...
    public function getUnreadNotificationStudent($studentid = null) {
        $date = date('Y-m-d');
        $sql  = " SELECT send_notification.`title` ";
        $sql .= " , send_notification.`date` ";
        $sql .= " FROM send_notification "; 
        $sql .= " LEFT JOIN read_notification ON  read_notification.student_id=" . $this->db->escape($studentid) . " AND read_notification.notification_id = send_notification.id "; 
        $sql .= " WHERE  send_notification.`visible_student`='yes' "; 
        $sql .= " AND read_notification.id IS NULL ";  
        $sql .= " AND DATEDIFF(send_notification.`publish_date`,'".$date."') <= 0 ";
        $query = $this->db->query($sql);        
        return $query->result_array();
    }

    public function getUnreadNotificationParent($parentid = null) {
        $date = date('Y-m-d');
        $sql  = " SELECT send_notification.`title` ";
        $sql .= " , send_notification.`date` ";
        $sql .= " FROM send_notification "; 
        $sql .= " LEFT JOIN read_notification ON  read_notification.parent_id=" . $this->db->escape($parentid) . " AND read_notification.notification_id = send_notification.id "; 
        $sql .= " WHERE  send_notification.`visible_parent`='yes' "; 
        $sql .= " AND read_notification.id IS NULL ";  
        $sql .= " AND DATEDIFF(send_notification.`publish_date`,'".$date."') <= 0 ";
        $query = $this->db->query($sql);        
        return $query->result_array();
