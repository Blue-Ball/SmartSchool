...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="box box-primary">
            ...
            <form  class="assign_teacher_form" action="<?php echo base_url(); ?>exam/evaluationlist" method="post" enctype="multipart/form-data">
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-12">
                ...
                <div class="col-md-12">
                    <div class="">
                        ...
                        <div class="box-body table-responsive">
                            <div class="download_label"> <?php echo $this->lang->line('examexam_submit_list'); ?></div>
                            <div >
                                <table class="table table-hover table-striped table-bordered example">
                                    <thead>
                                        <tr>
                                            <th><?php echo $this->lang->line('addmission_id') ?></th>
                                            <th><?php echo $this->lang->line('student_name') ?></th>
                                            <th><?php echo $this->lang->line('grade_point'); ?></th>
                                            <th><?php echo $this->lang->line('student_point') ?></th>
                                            <th><?php echo $this->lang->line('status'); ?></th>
                                            <th><?php echo $this->lang->line('marked_by'); ?></th>
                                            <th><?php echo $this->lang->line('submited_date'); ?></th>
                                            <th><?php echo $this->lang->line('evaluation_date'); ?></th>
                                            <th><?php echo $this->lang->line('result_status'); ?></th>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($homeworklist as $key => $homework) {
                                            ?>
                                            <tr>
                                                <td><?php echo $homework["submitid"] ?></td>
                                                <td><?php echo $homework["stdudentname"] ?></td>
                                                <td><?php echo $homework['gradepoint'] ?></td>
                                                <td><?php echo $homework["studentpoint"] ?></td>
                                                <td><?php echo empty($homework["studentpoint"]) ? '-' : ($homework["studentpoint"]-$homework['gradepoint']>=0 ? "Pass" : "Failed"); ?></td>
                                                <td><?php echo empty($homework["studentpoint"]) ? '-' : $homework["markedname"]; ?></td>
                                                <td><?php
                                                    if ($homework['submiteddate'] != "0000-00-00") { // &&  !empty($homework["studentpoint"])
                                                        echo date($this->customlib->getSchoolDateFormat(), $this->customlib->dateYYYYMMDDtoStrtotime($homework['submiteddate']));
                                                    }
                                                    else echo '-';
                                                    ?></td>
                                                <td><?php
                                                    if ($homework['evaluationdate'] != "0000-00-00" &&  !empty($homework["studentpoint"])) {
                                                        echo date($this->customlib->getSchoolDateFormat(), $this->customlib->dateYYYYMMDDtoStrtotime($homework['evaluationdate']));
                                                    }else echo '-';
                                                    ?></td>
                                                <td><?php echo $homework['evaluationdate'] != "0000-00-00" &&  !empty($homework["studentpoint"]) ? "Completed" : "Uncompleted"; ?></td>
                                                <td class="mailbox-date pull-right" >
                                                    <?php if ($this->rbac->hasPrivilege('homework_evaluation', 'can_view')) { ?>
                                                        <a data-placement="left" class="btn btn-default btn-xs" onclick="evaluation('<?php echo $homework['id']; ?>', '<?php echo $homework['student_id']; ?>');" title=""  data-toggle="tooltip"  data-original-title="<?php echo $this->lang->line('evaluation'); ?>">
...
<?php } ?>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="pwd"><?php echo $this->lang->line('start_date'); ?></label><small class="req"> *</small>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="pwd"><?php echo $this->lang->line('submit_date'); ?></label><small class="req"> *</small>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="exam_type"><?php echo $this->lang->line('exam_type'); ?></label>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="pwd"><?php echo $this->lang->line('exam_grade'); ?></label>
                                        <!-- <small class="req"> *</small> -->
                                        <select class="form-control" name="exam_grade" id="exam_grade">
                                            <option value=""><?php echo $this->lang->line('select') ?></option>
...
<div class="modal fade" id="evaluation" tabindex="-1" role="dialog" aria-labelledby="evaluation" style="padding-left: 0 !important">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="box-title"><?php echo $this->lang->line('evaluate_exam'); ?></h4>
            </div>
...
<div class="modal fade" id="homework_docs" tabindex="-1" role="dialog" aria-labelledby="evaluation" style="padding-left: 0 !important">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="box-title"><?php echo $this->lang->line('exam') . " " . $this->lang->line('assignments'); ?></h4>
            </div>
            <div class="modal-body pt0 pb0">
                <div class="row">
                    <div class="col-md-12">
...
<script type="text/javascript">
    ...
    function evaluation(homework_id, student_id) {
        $('#evaluation').modal('show');
        $('#evaluation_details').html("");
        $.ajax({
            url: '<?php echo base_url(); ?>exam/evaluation/' + homework_id + '/'+student_id,
...
<script type="text/javascript">
    ...
    function setExamTypeSelect(section_id, typeorgrade, select_control) 
    {
        {
            $('#' + select_control).html("");
    ...
    $(document).on('click', '.modal_form', function () {
        ...
        if (save_method == "edit") {
            ...
            $('#myModal').modal('show');
            $('#myModal .box-title').text('<?php echo $this->lang->line('edit_exam'); ?>');
        } else if (save_method == "add") {
            $('iframe').contents().find('.wysihtml5-editor').html("");
            $('#modal_record_id').val(0);
            $('#myModal .box-title').text('<?php echo $this->lang->line('add_exam'); ?>');
            $('#myModal').modal('show');
        } else {
        ...
        if (save_method == "edit") {
            $.ajax({
                ...
                success: function (res)
                {
                    ...
                    setExamTypeSelect(res.type_id, 'examtype', 'exam_type');
                    setExamTypeSelect(res.grade_id, 'examgrade', 'exam_grade');
